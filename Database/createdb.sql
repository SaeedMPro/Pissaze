...
CREATE TABLE client (
    ...
    wallet_balance     DECIMAL(12, 2) NOT NULL DEFAULT 0.00, --if we want to use Rial instead of $ we shall use "BIGINT NOT NULL DEFAULT 0"
...
CREATE TABLE vip_client (
    ...
    expiration_time TIMESTAMP WITH TIME ZONE NOT NULL, --SAME AS BEFORE 
...
CREATE TABLE discount_code (
    ...
    code_type       discount_enum NOT NULL --DASTKHOSH ! but be aware that it is static and if we want to change public code we will have problem !! 
);
...
CREATE TABLE private_code (
    ...
    client_id   INT NOT NULL,
    time_stamp  TIMESTAMP DEFAULT CURRENT_TIMESTAMP, --SAME AS BEFORE
    FOREIGN KEY (client_id) REFERENCES client (client_id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE refers (
    referee_id  VARCHAR(16) PRIMARY KEY, 
    referrer_id VARCHAR(16) NOT NULL,
